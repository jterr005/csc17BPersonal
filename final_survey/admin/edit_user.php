...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (empty($errors)) { // If everything's OK.
		...
		$q = "SELECT user_id FROM entity_survey_users WHERE email='$e' AND user_id != $id";
		$r = @mysqli_query($dbc, $q);
		if (mysqli_num_rows($r) == 0) {

			// Make the query:
			$q = "UPDATE entity_survey_users SET first_name='$fn', last_name='$ln', email='$e' WHERE user_id=$id LIMIT 1";
			$r = @mysqli_query ($dbc, $q);
...
$q = "SELECT first_name, last_name, email FROM entity_survey_users WHERE user_id=$id";		
$r = @mysqli_query ($dbc, $q);
