...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (empty($errors)) { // If everything's OK.
		...
		$q = "SELECT user_id FROM entity_survey_users WHERE (email='$e' AND pass=SHA1('$p') )";
		...
		if ($num == 1) { // Match was made.
			...
			$q = "UPDATE entity_survey_users SET pass=SHA1('$np') WHERE user_id=$row[0]";		
			$r = @mysqli_query($dbc, $q);
			...
			if (mysqli_affected_rows($dbc) == 1) { // If it ran OK.
				...
				// Public message:
				echo '<div style="text-align:center"><h1>System Error</h1>
...
<form action="password.php" method="post">
	<p>Email Address: <input type="text" name="email" size="20" maxlength="60" value="<?php if (isset($_POST['email'])) echo $_POST['email']; ?>" pattern="^[_A-Za-z0-9-]+(\.[_A-Za-z0-9-]+)*@[A-Za-z0-9]+(\.[A-Za-z0-9]+)*(\.[A-Za-z]{2,})$" /> </p>
